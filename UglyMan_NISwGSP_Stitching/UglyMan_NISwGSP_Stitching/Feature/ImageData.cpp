...
const vector<LineData> & ImageData::getLines() const {//LSD算法提取线特征
    if(img_lines.empty()) {
        const Mat & grey_image = getGreyImage();

        Mat mask = Mat::ones(grey_image.size(), CV_8UC1);
        Ptr<BinaryDescriptor> lbd = BinaryDescriptor::createBinaryDescriptor();
        //(*lbd)(grey_image, mask, keylines, descr, false, false);
        Ptr<LSDDetector> lsd = LSDDetector::createLSDDetector();
        
        lsd->detect(grey_image, keylines, 2, 2, mask);
        lbd->compute(grey_image, keylines, descr);

        const int line_count = (int)keylines.size();
        /*
        ...
        */
        ...
        for(int i = 0; i < line_count; ++i) {
            lines_points[0].emplace_back(keylines[i].getStartPoint().x, keylines[i].getStartPoint().y);
            lines_points[1].emplace_back(keylines[i].getEndPoint().x, keylines[i].getEndPoint().y);
            //lines_length.emplace_back(norm(lines_points[1][i] - lines_points[0][i]));
            lines_length.emplace_back(double(keylines[i].lineLength));
            lines_width.emplace_back(double(keylines[i].size));
        }
...
#ifndef NDEBUG
        Mat line_feature_img = Mat::zeros(grey_image.rows, grey_image.cols, grey_image.type());
        //Mat line_feature_img = img.clone();
        for(int i = 0; i < line_count; i++){
            Scalar color(rand() % 256, rand() % 256, rand() % 256);
            line(line_feature_img, lines_points[0][i], lines_points[1][i], color, 1);
        }
        imwrite(*debug_dir + "LSD_line-result-" + file_name + file_extension, line_feature_img);
        /*
...
const vector<Point2> & ImageData::getFeaturePoints() const {
    ...
    Mat feature_img = img.clone();
    for(int i = 0; i < feature_points.size(); ++i) {
        Scalar color(rand() % 256, rand() % 256, rand() % 256);
        circle(feature_img, feature_points[i], 5, color, 1, LINE_AA);
    }
    imwrite(*debug_dir + file_name + "-feature_points" + ".png", feature_img);
...
const vector<KeyLine> & ImageData::getKeyLines() const{
    if(keylines.empty()){
        img_lines = getLines();
    }
    return keylines;
...
const Mat & ImageData::getLineFeatureDescriptor() const{
    if(keylines.empty()){
        img_lines = getLines();
    }
    return descr;
