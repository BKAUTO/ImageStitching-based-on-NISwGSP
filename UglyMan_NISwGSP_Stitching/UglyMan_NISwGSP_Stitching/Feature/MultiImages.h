...
public:
    ...
    const vector<vector<vector<DMatch> > >  & getLineFeaturePairs() const;
    const vector<vector<vector<KeyLine> > > & getLineFeatureMatches() const;
    ...
    vector<vector<vector<InterpolateVertex> > >  get_lsd_mesh_intersection(vector<vector<pair<Point2f, Point2f>>> &endpoints) const;
    vector<vector<vector<pair<pair<cv::Point,cv::Point>,pair<cv::Point,cv::Point>>>>> get_line_match_result() const;
    vector<vector<Mat> >  getFun() const;
    vector<vector<vector<pair<Point2f,Point2f>>>> get_line_match_sample_points() const;

    int get_all_line_match_count() const;
    vector<vector<Rect2d>> get_overlap_rect() const;
...
private:    
    ...
    void initialLineFeatureMatchesSpace() const;
    void initialOverlapRect() const;
    
    vector<pair<int, int> > getInitialFeaturePairs(const pair<int, int> & _match_pair) const;

    vector<DMatch> getInitialLineFeatureMatches(const pair<int, int> & _match_pair) const;
    ...
    vector<DMatch> getLineFeatureMatchesBySequentialRANSAC(const pair<int, int> & _match_pair,
    ...
    mutable vector<vector<vector<DMatch> > > line_feature_pairs;
    mutable vector<vector<vector<KeyLine> > > line_feature_matches;
    mutable vector<DMatch> matches;
    mutable vector<KeyLine> lbd_octave1, lbd_octave2;
    mutable int all_line_match_count=0;
    mutable vector<vector<vector<pair<Point2f,Point2f>>>> line_match_sample_points;
    mutable vector<vector<vector<pair<pair<cv::Point,cv::Point>,pair<cv::Point,cv::Point>>>>> line_match_result;
    mutable vector<vector<Rect2d>> overlap_rect;
    mutable vector<vector<Mat> > fun;
